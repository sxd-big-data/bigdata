...
@Service
public class AclTreeNodeService extends BaseService{
	
	

	@Autowired
	private AclTreeNodeRepository treeNodeRep;
	...
	public void syncMysqlToHdfs(Boolean overWrite,String tableName){
		...
		//获取数据
		Map map = getDataByTransMeta(overWrite, tableName, null);
		...
		if(overWrite) {
			try {
				if(HDFSUtils.existFile(uploadFilePath)) {
					//将数据导入到临时表
					treeNodeRep.loadOverData(uploadFilePath);
				}
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
